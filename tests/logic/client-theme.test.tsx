...
describe('app client theme', () => {
  ...
  it('omits an unknown preset and falls back from its saved ID to Compact', async () => {
    localStorage.setItem('nocobase:crm:theme:preset', 'forest');
    render(
      <AppThemeProvider>
        <ThemeSettings />
      </AppThemeProvider>,
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Appearance' }));
    expect(
      screen.queryByRole('radio', { name: 'Forest' }),
    ).not.toBeInTheDocument();
    expect(screen.getByRole('radio', { name: 'Compact' })).toBeChecked();
    expect(screen.getByRole('radio', { name: 'Default' })).toBeInTheDocument();
    // The application's own presets are offered alongside the template's.
