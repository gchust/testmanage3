...
describe('shell translations', () => {
  it('updates header, footer, tooltips and accessible labels without remounting', async () => {
    ...
    expect(screen.getByText('Test management workspace')).toBeVisible();
    expect(screen.getByText('Test progress tracking')).toBeVisible();
    await act(() => runtime.changeLanguage('zh-CN'));
    expect(screen.getByText('测试管理工作区')).toBeVisible();
    expect(screen.getByText('测试进展统计')).toBeVisible();
    expect(
      screen.getByText('汇总功能点、Example 覆盖与质量评分。'),
    ).toBeVisible();
